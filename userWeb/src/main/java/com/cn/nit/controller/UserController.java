package com.cn.nit.controller;

import com.alibaba.fastjson.JSONObject;
import com.cn.nit.entity.User;
import com.cn.nit.service.UserDubboService;
import com.cn.nit.util.FormUtil;
import com.cn.nit.util.JsonUtil;
...
public class UserController {
	...
	public void initAdmin() {
		boolean isExist = userDubboService.isExist();
		if(!isExist){
			User user = new User();
	...
	public JSONObject dologin(HttpServletRequest request)
	{
		...
		User user = userDubboService.get("userName", username);
	...
	public JSONObject isExist4Username(HttpServletRequest request,String username){
		...
		if(!userDubboService.isExist(username)){//存在
...
//		session.setMaxInactiveInterval(60);
	...
	public JSONObject editUser(HttpServletRequest request,Integer id,String sex,String email,String phone){
		User user = userDubboService.get(id);
		user.setUserSex(sex);
		user.setEmail(email);
		user.setPhone(phone);
		userDubboService.update(user);
		return JsonUtil.success();
	}
...
//
	...
	*/
	...
	/**
	 * @Description: 判断原密码是否正确
	 * @author ding
	 * @date 2016年12月8日 下午6:09:02 
	 * @param request
	 * @param id
	 * @param pwd
	 * @return  
